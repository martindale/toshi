#!/usr/bin/env ruby
#
# Ressurrected orphan blockchain re-org test:
#
...
# for orphan block 5 redeem the same output from block 1 and send to two new addresses

new_tx = Bitcoin::Protocol::Tx.new
new_tx.add_in Bitcoin::Protocol::TxIn.new(prev_tx.binary_hash, prev_tx_output_index, 0)

key = blockchain.new_key("third spend to")
new_tx.add_out Bitcoin::Protocol::TxOut.value_to_address(value/2, key.addr)

key = blockchain.new_key("fourth spend to")
new_tx.add_out Bitcoin::Protocol::TxOut.value_to_address(value/2, key.addr)
